...
router.post("/sell", async (req: Request, res: Response) => {
  try {
    ...
    const pogsResult: { rows: { price: number }[] } =
    ...
    const checkResult: { rows: { stock: number }[] } =
      await prisma.$queryRawUnsafe(checkQuery, checkParams);
    ...
    const updateResult: { rows: { stock: number }[] } =
    ...
    if (updateResult.rows[0].stock === 0) {
      ...
      await prisma.$queryRawUnsafe(deleteQuery, deleteParams);
    }
